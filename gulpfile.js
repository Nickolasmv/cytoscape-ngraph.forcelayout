...
gulp.task('version', function( next ){
  var now = new Date();
  version = '1.3.8'//process.env['VERSION'];
...
gulp.task('push', shell.task([
  'git add -A',
  'git commit -m "pushing changes for v 1.3.8 release" || echo Nothing to commit',
  'git push || echo Nothing to push'
]));

gulp.task('tag', shell.task([
  'git tag -a 1.3.8 -m "tagging v1.3.8"',
  'git push origin 1.3.8'
]));
